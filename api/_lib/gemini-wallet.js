...
function geminiNeedsSetupBalance() {
  return {
    ...
    kind: "usd",
    label: "—",
    detail: "Log a top-up below, or set exact balance from AI Studio Billing (Google has no live balance API like Suno).",
...
function geminiLedgerBalance({ toppedUpUsd, usageAllUsd }) {
  ...
  return {
    ...
    detail: `${fmtUsdLabel(toppedUpUsd)} topped up − ${fmtUsdLabel(usageAllUsd)} tracked usage`,
    billingUrl: "https://aistudio.google.com/billing",
    sharedWallet: true,
    walletIds: [GEMINI_WALLET_ID, "lyria"],
  };
...
function geminiSnapshotBalance({ snapshotUsd, syncedAt, usageSince, syncedBy = "" }) {
  ...
  return {
    ...
    syncedAt,
    label: fmtUsdLabel(remaining),
    detail: `Set to ${fmtUsdLabel(snapshotUsd)} (${syncLabel}${by}) − ${fmtUsdLabel(usageSince)} since`,
    billingUrl: "https://aistudio.google.com/billing",
    sharedWallet: true,
    walletIds: [GEMINI_WALLET_ID, "lyria"],
  };
