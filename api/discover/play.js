...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const rowRes = await selectFromTable(
      `user_songs?id=eq.${encodeURIComponent(songId)}&select=id,user_id,public_on_profile,published_at`,
    );
    ...
    // Discover survival scoring paused — do not read/write discover_score columns
    // until supabase/user_songs_discover_survival.sql is applied in production.
    const score = -100;
