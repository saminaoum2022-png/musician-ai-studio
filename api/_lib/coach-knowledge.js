...
- ARABIC ACCENT — best workflow: pick **Dialect** (e.g. Lebanese), tap **✦ Generate** on lyrics (uses **Suno** so the words match how it sings), then **Generate song**. **Add vowel marks** still uses Gemini (full lyrics are too long for Suno’s lyrics API). Style tags like **Levantine Pop** help. Colloquial Lebanese words beat formal فصحى.
