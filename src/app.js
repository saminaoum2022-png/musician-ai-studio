...
async function createPersonaForSong({
  ...
  try {
    ...
    if (!r.ok) {
      ...
      let hint = "";
      // Code 500 from Suno is often "song wasn't generated by V4+
      // engine" or "song not fully ready". Give the user something
      // they can act on rather than just "internal error".
      const isInternal = /internal error|code 500|Suno hit/i.test(baseMsg);
      if (isInternal) {
        hint = " — songs need to be fully finished and generated with V4+ to become a persona.";
      }
...
void (async () => {
  ...
  if (authSession?.user?.id) {
    ...
    // Migration safety net: if the cloud row was written before this
    // fix landed and is still "guest", upgrade it now to a derived
    // username and persist back to Supabase so the row stops
    // colliding with other users.
