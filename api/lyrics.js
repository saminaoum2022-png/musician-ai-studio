...
module.exports = async function handler(req, res) {
  try {
    ...
    const prompt = buildPrompt({ seed, style, mode, nonce });
    const complianceTerms = extractComplianceTerms({ seed, style });

    const debug = {};
    const sunoKey = process.env.SUNO_API_KEY;
    if (sunoKey) {
      const sunoResult = await trySunoLyrics({ sunoKey, prompt });
      if (sunoResult?.ok) {
    ...
    if (geminiKey) {
      ...
      if (gemResult?.ok) {
        ...
        return json(res, 200, {
          ...
          debug: { nonce, suno: debug.suno || "failed", gemini: "ok" },
        });
      }
    ...
    return json(res, 502, {
      error: "Lyrics provider unavailable (Suno/Gemini).",
      provider: "none",
      debug: { ...debug, nonce },
    });
  } catch (e) {
...
async function tryGeminiLyrics({ geminiKey, prompt }) {
  ...
  return { ok: true, lyrics: out };
}

function buildPrompt({ seed, style, mode, nonce }) {
  if (mode === "arrange") {
    return [
